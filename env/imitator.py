...
class Imitator(Runner):
    ...
    def reset(self):
        ...
        return {'obs': obs[0], 'gt_action': self.action_waiting_list[0]}
    ...
    def step(self, raw_action):
        if None in self.action_waiting_list:
            ...
            return {'obs': obs[0], 'gt_action': self.action_waiting_list[idx]}, np.asarray(0.0), np.asarray(False), {}
        ...
        try:
            ...
            return {'obs': obs[0], 'gt_action': self.action_waiting_list[self.id]}, np.asarray(0.0), np.array(True), {}
        finally:
            pass
        ...
        return {'obs': obs[0], 'gt_action': self.action_waiting_list[self.wait_to_play]}, np.asarray(0.0), np.asarray(False), {}