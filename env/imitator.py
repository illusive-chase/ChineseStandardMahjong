...
class Imitator(Runner):
    ...
    def reset(self, idx=None):
        ...
        return {'obs': obs[0], 'mask': obs[1], 'gt_action': self.action_waiting_list[0], 'player': 0}
    ...
    def step(self, raw_action):
        if None in self.action_waiting_list:
            ...
            if obs[1].sum() > 1:
                return {'obs': obs[0], 'gt_action': self.action_waiting_list[idx], 'mask': obs[1], 'player': idx}, np.asarray(0.0), np.asarray(False), {}
        ...
        try:
            ...
            return {'obs': obs[0], 'mask': obs[1], 'gt_action': self.action_waiting_list[self.id], 'player': self.id}, self.rew, np.array(True), {}
        finally:
            pass
        ...
        if obs[1].sum() > 1:
            return {'obs': obs[0], 'mask': obs[1], 'gt_action': self.action_waiting_list[self.wait_to_play], 'player': self.wait_to_play}, np.asarray(0.0), np.asarray(False), {}
        return self.step(None)